...
INSERT INTO {{.scratch_schema}}.etl_tstamps (
  ...
  FROM
    {{.input_schema}}.events
  WHERE
    etl_tstamp NOT IN (SELECT etl_tstamp FROM {{.output_schema}}.engagement_manifest ORDER BY 1)
    AND etl_tstamp BETWEEN
      (SELECT MAX(etl_tstamp) FROM {{.output_schema}}.engagement_manifest) - INTERVAL '1 week' -- to ensure that any quarantined data is not missed
      AND
      (SELECT MAX(etl_tstamp) FROM {{.output_schema}}.engagement_manifest) + INTERVAL '{{.update_cadence}}'
